...
class Utility:
    ...
    def assign_model_raster(tasks):
        ...
        for task in tasks:
            for model in task.get_models().values():
    ...
    @staticmethod
    def get_global_time_scale(tasks):
        hcf_time = []
        for model in Utility.get_models(tasks):
    ...
    @staticmethod
    def get_triggered_task_models(tasks):
        triggered_models = {}
        for model in Utility.get_models(tasks):
            if model.is_trigger_based():
                if model.get_triggered_connection_priority() in triggered_models.keys():
                    raise Exception(f'Triggered connection priority {model.get_triggered_connection_priority()} already exists')
                triggered_models[model.get_triggered_connection_priority()] = model
        
        return triggered_models
    
    @staticmethod
    def get_common_raster(connected_models):
        hcf = []
    ...
    @staticmethod
    def reconfigure_task(tasks):
        for task in tasks:
            task.reconfigure_priorities()
            if task.name == 'Triggered Task':
                for model in task.get_models().values():
                    model.set_raster(task.raster , True)